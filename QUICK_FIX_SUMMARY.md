# 🔧 Быстрая сводка исправлений

## ✅ Что исправлено

### 1. Порт изменен на 5001 везде
- ❌ Было: разные порты (5000/5001 вперемешку)
- ✅ Стало: **5001 везде**

**Изменено в:**
- `backend/.env` → `PORT=5001`
- `backend/src/server.js` → `const PORT = process.env.PORT || 5001`
- `mobile/.env` → `http://192.168.0.191:5001/api`
- `mobile/app.json` → `"apiUrl": "http://192.168.0.191:5001/api"`
- `mobile/services/api.ts` → `'http://192.168.0.191:5001/api'`
- Вся документация

### 2. Backend слушает на всех интерфейсах
```javascript
app.listen(PORT, '0.0.0.0', () => { ... });
```
Теперь backend доступен не только на localhost, но и по IP.

### 3. CORS настроен правильно
```javascript
// Разрешены запросы из локальной сети
if (origin && origin.startsWith('http://192.168.')) {
  callback(null, true);
}
```

Также в списке allowed origins:
- `http://localhost:5173` (web)
- `http://localhost:8081` (mobile dev server)
- `http://192.168.0.191:8081` (mobile на устройстве)

## 🚀 Как проверить

### Шаг 1: Запустить backend
```bash
cd backend
npm run dev
```

Вы увидите:
```
🚀 Сервер запущен на порту 5001 и доступен на всех сетевых интерфейсах
📱 Мобильное приложение: http://192.168.0.191:5001/api
💻 Веб-приложение: http://localhost:5001/api
```

### Шаг 2: Проверить доступность
```bash
# Локально
curl http://localhost:5001/api/health

# По IP (если это не работает - проблема в firewall)
curl http://192.168.0.191:5001/api/health
```

### Шаг 3: Запустить mobile
```bash
cd mobile
pnpm start
```

## ⚠️ Возможные проблемы

### Проблема: `curl http://192.168.0.191:5001/api/health` не работает

**Причина:** Firewall блокирует порт 5001

**Решение:**

#### Windows (PowerShell от администратора):
```powershell
New-NetFirewallRule -DisplayName "ToolManager Backend" -Direction Inbound -LocalPort 5001 -Protocol TCP -Action Allow
```

#### Linux:
```bash
sudo ufw allow 5001/tcp
```

#### macOS:
System Preferences → Security & Privacy → Firewall → Firewall Options → Добавить Node.js

### Проблема: CORS ошибка "No 'Access-Control-Allow-Origin' header"

**Это уже исправлено!** Проверьте что backend перезапущен с новым кодом.

Проверьте preflight запрос:
```bash
curl -X OPTIONS http://192.168.0.191:5001/api/auth/login \
  -H "Origin: http://localhost:8081" \
  -H "Access-Control-Request-Method: POST" \
  -v
```

Должны увидеть в ответе:
```
Access-Control-Allow-Origin: http://localhost:8081
Access-Control-Allow-Credentials: true
```

## 📝 Файлы для справки

- `TEST_INSTRUCTIONS.md` - Подробные инструкции по тестированию
- `MOBILE_SETUP.md` - Полная документация по настройке
- `CHANGES_SUMMARY.md` - Техническое описание изменений

## 🎯 Чеклист перед тестированием

- [ ] Backend .env содержит `PORT=5001`
- [ ] Mobile .env содержит `EXPO_PUBLIC_API_URL=http://192.168.0.191:5001/api`
- [ ] Mobile app.json содержит `"apiUrl": "http://192.168.0.191:5001/api"`
- [ ] Backend перезапущен
- [ ] Firewall разрешает порт 5001
- [ ] Телефон и компьютер в одной Wi-Fi сети

Если всё отмечено - можно тестировать! 🚀
